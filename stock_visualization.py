...
def fetch_stock_data(symbol, start_date, end_date, max_retries=3):
    ...
    for retry_count in range(max_retries):
        try:
            ...
            # Method 1: Try yfinance with direct download
            #st.info(f"Attempt {retry_count+1}: Fetching data for {symbol} from {start_date.date()} to {adjusted_end_date.date()}")
            data = yf.download(
                tickers=symbol,
                start=start_date,
            ...
            if not data.empty and len(data) > 0:
                ##st.success(f"Successfully retrieved {len(data)} days of data")
                return data
                
            # Method 2: If yfinance fails, try Ticker.history approach
            ...
            if not hist_data.empty and len(hist_data) > 0:
                #st.success(f"Successfully retrieved {len(hist_data)} days of data via Ticker.history")
                return hist_data
                
            # Method 3: Last resort - direct API call
                ...
                try:
                    ...
                    if 'chart' in json_data and 'result' in json_data['chart'] and json_data['chart']['result']:
                        ...
                        if not df.empty and len(df) > 0:
                            #st.success(f"Successfully retrieved {len(df)} days of data via direct API")
                            return df
                        ...
                        if df:
                            for _, row in df.iterrows():
                                cursor.execute(
                                    ...
                                    (
                                        symbol, row['Date'].strftime('%Y-%m-%d'), row['Close'], row['Prev_ratio'],
                    ...
                    st.warning(f"API parsing error: {str(api_error)}")
            ...
            st.warning(f"Attempt {retry_count+1} failed: {str(e)}. Retrying in {wait_time} seconds...")
            time.sleep(wait_time)
    
    st.error(f"Could not retrieve data for {symbol} after multiple attempts")
...
def calculate_daily_change(data):
    ...
    # Calculate previous close ratio for DB
...
def create_stock_visualization(stock_data, symbol, display_options):
    ...
    monthly = stock_data.resample('M')
    monthly_diff = (monthly['High'].max() - monthly['Low'].min()).round(1)
    ...
    if display_options.get('daily_change', False):
        ...
        fig.add_trace(
            go.Scatter(
                x=stock_data.index,
                y=stock_data['Daily_Change_MA20'],
...
def show_investment_data():
    ...
    # Database connection setup - put your actual credentials here
    db_config = {
        "host": "localhost",
        "user": "your_username", 
...
def stock_chart(ticker='TSLA'):
    # App title
    #st.title('US Stock Data Visualization')

    # 신규 종목 추가 UI
    st.subheader('종목 리스트에 신규 종목 추가')
    col1, col2, col3 = st.columns([2,3,1])
    with col1:
        new_us_symbol = st.text_input('심볼 입력', '').upper().strip()
    with col2:
        new_us_name = st.text_input('종목명 입력', '').strip()
    with col3:
        if st.button('추가'):
            if new_us_symbol and new_us_name:
                add_us_stock(new_us_symbol, new_us_name)
                st.success(f'{new_us_symbol} - {new_us_name} 추가되었습니다.')
                st.experimental_rerun()
            else:
                st.warning('심볼과 종목명을 모두 입력하세요')

    # User inputs - use session state if available
    us_list = get_us_stock_list()  # list of (symbol, name)
    options = [f"{s} - {n}" for s, n in us_list]
    default_sym = st.session_state.get('selected_ticker', ticker)
    default_index = next((i for i, (s, _) in enumerate(us_list) if s == default_sym), 0)
    selected = st.selectbox('Select Stock (symbol - name)', options, index=default_index, key='us_stock_symbol')
    ...
    selected_period = st.selectbox('Select Display Time Period', options, index=default_index, key='us_stock_period')
    ...
    with col6:
        show_bb_width = st.checkbox('BB Width', value=False)
    with col7:
        show_rsi = st.checkbox('RSI', value=True)
    with col8:
        show_daily_change = st.checkbox('Daily Change', value=False)
    with col9:
        show_stochastic = st.checkbox('Stochastic', value=False)
    with col10:
        show_vol_ma20 = st.checkbox('Vol MA20', value=False)
    with col11:
        show_vol_ma50 = st.checkbox('Vol MA50', value=False)
    ...
    display_options = {
        ...
        'Volume_MA20': show_vol_ma20,
        'Volume_MA50': show_vol_ma50
    }
    ...
    try:
        if not symbol:
            ...
            with st.spinner(f'Loading data for {symbol}...'):
                ...
                full_stock_data = fetch_stock_data(symbol, start_date, end_date)
...
def display_exchange_rates():
    st.header("주요 통화 환율 정보")
    
    # 기본 설정값
    default_currencies = ["KRW=X", "JPYKRW=X", "JPY", "EURKRW=X"]
    all_currencies = ["KRW=X", "JPYKRW=X", "JPY", "EURKRW=X"]
    max_points = 30  # 기본 30일
    
    # 환율 설정 섹션 (탭 내부에 표시)
    with st.expander("환율 표시 설정", expanded=True):
        col1, col2 = st.columns(2)
        ...
        with col1:
            ...
            st.caption(f"최근 {max_points}일치 데이터를 표시합니다")
        
        # 오른쪽 열 - 통화 설정
        with col2:
            st.subheader("통화 설정")
            # multiselect에서 selectbox로 변경하여 하나만 선택 가능하게 함
            selected_currency = st.selectbox(
                "표시할 통화 선택 (KRW 기준)",
                all_currencies,
                index=0  # 기본값으로 USD 선택
            )
            
            # 선택된 통화를 리스트로 변환 (기존 API 호환성 유지)
    ...
    # 환율 데이터 가져오기
    try:
        # exchange_rate_visualization.py 모듈에서 import한 함수 사용
        exchange_data = get_exchange_rates_from_yahoo(currencies=selected_currencies, max_points=max_points)
        #exchange_data = get_exchange_rates_from_ecos(currencies=selected_currencies, max_points=max_points)
        ...
        # 일별 변동성 그래프 추가
        #st.subheader("일별 변동률")
        #visualize_exchange_volatility(exchange_data)

        # 환율 추이 그래프와 일별 변동률을 통합 그래프로 표시
        st.subheader("환율 추이 및 변동률")
        visualize_combined_exchange_data(exchange_data)
        
    except Exception as e:
        st.error(f"환율 데이터 처리 중 오류 발생: {e}")
